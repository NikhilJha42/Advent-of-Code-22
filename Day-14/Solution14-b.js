...
const y_limit = y_max + 150;

const inputMatrix = [];
for(let i=0; i < x_max + 3; i++){
    inputMatrix.push([]);
...
// Step 3.5: We now have a floor on the row x_max + 2:

for(let k=0; k < y_limit; k++){
    cave.setValueAt('#', x_max + 2, k);
}
...
console.log(counter);
// 17929 is too low. 23389 is too low. 23390?

//Outputting diagram to test.txt
for(let i=0; i < x_max + 3; i++){
    let str = '';
    for(let j=0; j < y_limit; j++){
        str = str + cave.valueAt(i, j);
    }
    console.log(str);
}